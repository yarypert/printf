...
if (lst->preci != -1)
	{
		if(lst->mode_l == 1)
			...
			lst->str = ft_strreplace(lst->str, lst->str, precision_int
			(ft_ltoa_base(va_arg(env->args, long long int), 8), lst->preci));
		else
			lst->str = ft_strreplace(lst->str, lst->str, precision_int
			(ft_itoa_base(va_arg(env->args, int), 8), lst->preci));
	}
	else
	{
		if(lst->mode_l == 1)
		...
		lst->str = ft_strreplace(lst->str, lst->str,
			ft_ltoa_base(va_arg(env->args, long long int), 8));
		else
		lst->str = ft_strreplace(lst->str, lst->str,
			ft_itoa_base(va_arg(env->args, int), 8));
	}
	if(ft_strcmp(lst->indic , "-") == 0)
		lst->larg = ft_strjoin("-", lst->larg);
